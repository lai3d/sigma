...
pub async fn sync_single_zone(
    ...
    let domain_resp = client
        .get(format!("{GODADDY_API_BASE}/domains/{zone_name}"))
        .header("Authorization", &auth)
        .send()
        .await;
    ...
    let domain_expires = if let Ok(resp) = domain_resp {
        resp.json::<GdDomain>()
            .await
            .ok()
            .and_then(|d| d.expires.as_deref().and_then(|s| s.parse::<chrono::DateTime<chrono::Utc>>().ok()))
    } else {
        None
    };
    ...
    let records_resp = client
        .get(format!("{GODADDY_API_BASE}/domains/{zone_name}/records"))
        .header("Authorization", &auth)
        .send()
        .await;
